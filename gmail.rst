...
.. warning::
   このドキュメントの情報は古くなっており、現在の Gmail のインターフェースや機能とは異なる可能性があります。
   最新の情報については、Gmail のヘルプや NVDA の公式ドキュメントをご参照ください。
...
ブラウザで使用できる Gmail は、 2015年ごろから Firefox と NVDA をサポートするようになりました。
...
-  チャット、カレンダーなど、他のサービスと連携
-  PC
   にインストールされたメールアプリや、スマートフォンのアプリからも利用できる

ここでは主に Firefox による Gmail の操作を説明しますが、Google Chrome でも同様に NVDA を使って Gmail を利用できます。
このセクションを執筆した時点での内容であることをお断りしておきます。

晴眼者が使う Web
サービスとまったく同じ画面をスクリーンリーダーで操作できること、
Gmail のような高機能の Web
サービスがスクリーンリーダーに対応していることが、広く知られることを願っています。
...
Firefox または Chrome の設定は以下を想定しています。（ここでは Firefox の設定項目名を記載します）
...
NVDA と Firefox または Chrome の組み合わせで Gmail
...
「ブラウズモード」には Gmail の操作に役立つ特別な機能があります。
...
基本的に、NVDA の1文字ナビゲーション（Hで見出し、Bでボタンなど）は **ブラウズモード** で使用し、Gmail 独自のキーボードショートカット（cで作成、rで返信など）は **フォーカスモード** で使用します。

以下では「英字キーを入力に使用」はメールアドレスの入力に使います。
「フォーカスモード」は件名やメッセージ本文の入力、および Gmail ショートカットキーの使用に使います。
それ以外は「1文字ナビゲーションを使用」のブラウズモードで操作します。 Gmail
のキーボードショートカットは有効にしますが、ここでは主に NVDA の1文字ナビゲーションを使った操作を説明します。ショートカットキーは NVDA での基本的な操作に慣れた方向けの情報として紹介します。

受信トレイと設定ページ
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Firefox や Chrome でアドレスバー（「URLまたは検索語句を入力します エディット」など）に移動するには
Ctrl+L を押します。
...
Firefox や Chrome のメニューバーと Gmail のページ内容は Alt
キーを押すたびに行ったりきたりできます。
...
B または Shift+B を使って「作成 ボタン」でスペースを押します。（Gmail のショートカットキー ``c`` でも作成できます）
...
下矢印キーで「リンク 返信 または」に移動して Enter を押します。（スレッド表示中に ``r`` キーを押すことでも返信できます）
...
ボタン(B)で「アーカイブ ボタン」に移動してスペースを押します。（スレッド表示中または受信トレイでスレッドを選択中に ``e`` キーを押すことでもアーカイブできます）
...
エディット(E)で「検索 エディット サブメニュー」に移動します。（``/`` キーを押すと直接検索ボックスにフォーカスします）
...
-  Enter で実行します。（``g`` の後に ``i`` を押すショートカットでも受信トレイに戻れます）

以上で Gmail の基本的な操作を紹介しました。
マスターできたら、自分以外のメールアドレスに送信してみましょう。

なお「スペルの間違いを入力中に音で報告」は、現在は日本語には対応していませんが、便利な機能です。
