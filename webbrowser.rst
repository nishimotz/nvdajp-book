...
スクリーンリーダーでWebページを「読む」場面では、Tabキーでフォーカス移動を繰り返すよりも、
ブラウズモードの **1文字ナビゲーション**（見出し、テーブル、ランドマークなど）を中心にしたほうが
目的の情報に到達しやすいことが多いです。

- Tabキーは「フォーカス可能な操作対象（リンク、ボタン、フォーム等）」だけを順に移動します。
  そのため、押しすぎるとページ本文から外れて **アドレスバーやツールバーなどのブラウザ操作UI** に移動してしまい、
  どこを読んでいるのか分からなくなることがあります。
- 一方、1文字ナビゲーション（例: H=見出し、T=テーブル など）は、ページ構造を辿るための操作です。

次のように、目的に応じて使い分けると安定します。

- **ドキュメントの構造把握・読解**: 1文字ナビ（H/T/ランドマーク）や上下矢印
- **操作の検証・フォーム入力・フォーカス順の確認（WCAG 2.4.3等）**: Tab（必要最小限）

（ここに図を入れたい：1文字ナビ中心の読解と、Tab中心の操作を使い分ける図）

Tabを押しすぎてブラウザ操作UIへ移動してしまったとき
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Tabを押しすぎてページ本文ではなくブラウザ操作UI（アドレスバー等）に移動してしまった場合は、
次のように「状態を整える」ことで復帰しやすくなります。

- NVDA+Tでウィンドウタイトルを確認し、ブラウザUIにいるかを把握する
- NVDA+スペースでブラウズモード/フォーカスモードを切り替え、意図した状態に揃える
- 必要なら Escape を1〜2回押して、開いているUI（候補リスト等）を閉じる

また、上下矢印キー等で移動しても反応がない、あるいは同じ読み上げを繰り返す場合は、
...
`「Web閲覧の操作」 <https://web.archive.org/web/20160309080140/http://nvdajp.osdn.jp/howto/manual/>`__ というリンクを探しましょう。（Wayback Machine のアーカイブ）
