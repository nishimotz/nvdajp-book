...
Firefox は必要に応じて利用してください。インストール方法の詳細は、公式サイトをご参照ください。

-  `Mozilla Firefox の Web サイト <https://www.mozilla.org/ja/firefox/>`__

（以降の説明は Edge / Chrome / Firefox で共通です）
...
.. note::
   ウィンドウタイトルの末尾（ブラウザ名）は環境によって異なります（例: Microsoft Edge / Google Chrome / Mozilla Firefox）。
...
.. note::
   Wayback Machine のページを開くと、最初はWaybackのヘッダ（例:「Wayback Machine」やURL入力欄）にフォーカスがあることがあります。
   その場合は 1文字ナビゲーションの D（ランドマーク）や H（見出し）で本文（例:「NVDA日本語版 操作ガイド」）まで移動できます。
