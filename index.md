...
NVDA は Windows の操作を音声や点字でガイドします。
また、Windows を直接操作しないで、ウィンドウに表示されている情報を読み取るための特別な機能も NVDA には備わっています。
...
例えば以下で表示されるのは、一般的な操作に対応した Windows のダイアログボックスです。
...
ここでは Windows 10 バージョン 1703 のスタートメニューを紹介します。
Windows 10 には「デスクトップモード」と「タブレットモード」があります。
...
#### 4.3.2 Windows Update 


#### 4.3.3 トラブルシューティングツール
...
ひきつづき Windows 10 バージョン 1703 の「設定」を使い、
今度は NVDA のレビューカーソルの使い方（オブジェクトナビゲーション）を紹介します。
...
Windows+I を押すと「設定」が開いて、以下のように報告されます：
