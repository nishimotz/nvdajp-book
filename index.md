...
### 3.2 Firefox で Gmail を使う

#### 3.2.1 Gmail の概要

ここから Gmail というサービスの使い方を紹介します。

[Google](https://www.google.co.jp/) が提供する Gmail （ジーメール）は Web ブラウザから使える高機能のメールサービスです。
...
この画面でメッセージ本文を最後まで確認できない場合や、返信をしたい場合には、
このメールを選択して「スレッド」という画面に移動します。

6列の場所で Enter を押します。
（リンクなので Enter を使いましたが、スペースでも実行できます）
...
Gmail のアカウントごとの設定によって内容は異なる場合があります。
