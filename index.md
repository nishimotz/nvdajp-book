...
### 3.11 Firefox で Gmail を使う

#### 3.11.1 Gmail の概要

ここから Gmail というサービスの使い方を紹介します。

[Google](https://www.google.co.jp/) が提供する Gmail （ジーメール）はスマートフォンや Web ブラウザから使える高機能のメールサービスです。
...
これ以降の説明は Firefox の URL 入力で [gmail.com](http://gmail.com) を開いて、ログインに成功した状態を想定しています。

現在のページのタイトルは Firefox のウィンドウのタイトルになので NVDA+T で確認できます。
まず「受信トレイ」で始まるタイトルのページについて説明します。
実際には受信トレイ以外の（例えば「下書き」の）一覧ページもほぼ共通です。

Firefox のメニューバーと Gmail のページ内容は Alt キーを押すたびに行ったりきたりできます。

「ファイル サブメニュー Alt+F 1の7」

「受信トレイ(未読の数) - (あなたのアドレス) - Gmail ドキュメント テーブル 未読あり ・・・」

確認できたら受信トレイにフォーカスを戻してください。

次に、ブラウズモードになっていることを確認するために、NVDA+スペース を押してください。
「ポン」という音がすれば大丈夫ですが、「ガシャ」という音がしたときには、
もういちど NVDA+スペース を押してください。

1文字ナビゲーションの B でボタンを探しましょう。
そのまま押すと下に向かってジャンプ、 Shift+B で上に戻るジャンプができます。
「次のボタンがありません」と言われたら NVDA+Tab で現在位置を確認します。
行きすぎたら Shift+B で戻りましょう。

ボタンは押してなにかを実行するための要素です。
スペースでボタンを押すことができます。

Gmail では「折りたたみボタン」や「メニューボタン」も登場します。

「設定 メニューボタン 折りたたみ サブメニュー」

を見つけてください。

見つかったらそこで Enter を押してください。

「展開 メニュー サブメニュー 表示間隔・・・」

ボタンを押すとその場所にメニューが開いて、その中の項目を最後まで読み上げます。
Shift で読み上げを中断できます。

Esc を押すと「メニューの外」と読み上げてメニューを閉じます。

メニューが開いた直後の状態で下矢印キーを押してみましょう。
...
「設定」という項目で Enter を押して、設定の画面に移動しましょう。

読み上げを途中で止めて NVDA+T でウィンドウのタイトルを確認、NVDA+Tab でフォーカスを確認します。

- NVDA+T 「設定 - (あなたのアドレス) - Gmail - Mozilla Firefox」
- NVDA+Tab 「全般 タブ フォーカス 選択」

ラジオボタンを探すために R を押してください。

- 「キーボード ショートカット OFF ラジオボタン チェック」
- 「キーボード ショートカット ON ラジオボタン チェックなし」

「チェックなし」のところでスペースを押します。

「チェック」と読み上げれば設定を変更できました。

ランドマークを単位にして移動する D を押します。

「ナビゲーション ランドマーク ボタン 変更を保存」

「ボタン キャンセル」

なお、この2つのボタンは左右に並んでいるので「画面レイアウト」だと上下矢印キーで順番に移動できません。

「変更を保存」または「キャンセル」でスペースを押すと
「使用不能」と読み上げられますが、すこし待つと「受信トレイ」の画面に戻ります。


ランドマークは Web ページの一部分に与えられた「役割」を示しています。
「受信トレイ」では「バナー」「ナビゲーション」「補足情報」「メインコンテンツ」といった役割が使われています。

1文字ナビゲーションで「受信トレイ」のページ全体を把握してみましょう。

受信トレイのページを開くと、新着メールがあれば最初の新着メールに、
新着メールがなければ「新着メールはありません」と書かれた行に移動しますが、ここはページの先頭ではありません。
Ctrl+Home でページの先頭の「ボタン 簡易HTML形式」に移動します。

ボタン(B)はどうやらたくさんあります。

ページの先頭に戻って、見出し(H)で移動してみます。
「検索」「ラベル」「メイン」が見つかります。
Gmail の設定で「チャット」を有効にしていると「ラベル」の次に「ハングアウト」という項目が表示されます。
今回は「チャット」を無効にしてかまいません。

ページの先頭に戻って、今度はランドマーク(D)で移動してみます。
「バナー」「検索」「ラベル」「メイン」が見つかります。

エディットフィールド(E)は「検索」だけ見つかります。

これらの要素が実際にどう並んでいるか、独自に調査した結果を下記に紹介します。
...
エディットフィールドはテキスト入力ができる要素です。
NVDA の「自動フォーカスモード」をチェックなしにしているので、
テキスト入力をするためにはエディットフィールドにフォーカスを移動したあとで NVDA+スペース を押します。
「ガチャ」という音がすることを確認します。
テキスト入力をやめるときは NVDA+スペース を押して「ポン」という音を確認します。


#### 3.11.3 Gmail でメールを作成する

- 「作成」ボタンに移動して Enter を押す
- 「To」欄に受信者を入力
- Tab キーを押して 「件名」欄に移動
- 件名を入力
- Tab キーを押してメール本文に移動
- メッセージを入力
- Tab キーを押して「送信」ボタンに移動
- Enter を押して送信を実行


#### 3.11.4 Gmail のスレッドページ

多くの要素は受信トレイのページと共通です。
...
- スレッドページで「返信」に移動して Enter を押す
- 新規メッセージを送信せずに閉じるには Esc キーを押す（ナビゲーションリストの「下書き」ラベルで作業を継続できる）


#### 3.11.6 メールの検索

- Gmail アクションバーの「Gmail検索」エディットボックスに移動
- 検索キーワードを入力して Enter を押す
- 検索結果としてスレッドのリストが表示される
- 受信トレイに戻るにはナビゲーションリストの「受信トレイ」を選択する


#### 3.11.7 ショートカットと1文字ナビゲーションの切り替え

Gmail のいくつかの操作にはキーボードショートカットが用意されています。
初期状態から有効になっているショートカットだけでなく、
ユーザーが設定した場合に有効になるスクリーンリーダー用の拡張ショートカット操作もあります。
...
NVDA のブラウズモードでは、英字キーの入力は Firefox の1文字ナビゲーションの操作として解釈されます。
たとえば c を押すとコンボボックスにジャンプします。

Gmail では新規メールの作成 (compose) に c というキーボードショートカットが割り当てられています。
NVDA をフォーカスモードに切り替えれば c というキー入力は1文字ナビゲーションではなく Gmail のショートカットとして解釈されます。

しかし、フォーカスモードでは、上下の矢印キーで確認できない要素がたくさんあり、あまり便利ではありません。

NVDA 2015.3 以降では、矢印キーの機能はブラウズモードのまま、1文字ナビゲーションを無効にして、英字キーを入力として使う機能が追加されました。

この切り替えは以下の操作で行います：

- NVDA+Shift+スペース：「英字キーを入力に使用」「1文字ナビゲーションを使用」を切り替える

ブラウズモードで Gmail のショートカットを使うときには「英字キーを入力に使用」に切り替えてください。
