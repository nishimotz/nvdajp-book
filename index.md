...
説明に使うのは Windows 10 バージョン 1703 の「設定」です。
...
### 4.A （資料）「設定プロファイル」を使った説明

この資料では、Windows 10 の「設定」ではなく
NVDA の「設定プロファイル」を使って「アクティブウィンドウの読み上げ」と
「レビューカーソル」「ナビゲーターオブジェクト」を説明しています。
設定プロファイルの機能そのものはここでは使いません。

Windows 10 を使っていない場合は、4.1 および 4.2 の内容のかわりに、
この手順でオブジェクトナビゲーションを学んでください。
