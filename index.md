...
まず「受信トレイのページ」の構成を説明しますが、実際には受信トレイ以外の（例えば「下書き」の）一覧ページもほぼ共通です。

ブラウズモードの1文字ナビゲーションを思い出してください。
ブラウズモードで K や H をそのまま押すと下に向かって次のリンクや見出しに、Shift と一緒に押すと上に向かってにジャンプできます。

Gmail で使う1文字ナビゲーションをここでまとめます：
...
Gmail では「バナー」「ナビゲーション」「補足情報」「メインコンテンツ」といったランドマークが使われています。
ランドマークと見出しでジャンプすれば、Gmail のページの中を効率的に移動できます。

エディットフィールドはテキスト入力ができる要素です。
エディットフィールドにフォーカスが移動すると NVDA はフォーカスモードに切り替わるので「ガチャ」という音がします。
エディットフィールドからフォーカスが離れるときには「ポン」という音がしてブラウズモードに戻ります。
自動的に切り替わらない場合は NVDA+スペース で切り替える必要があります。

ボタンは押してなにかを実行するための要素です。
Windows の作法ではボタンはスペースキーで押すのですが、
Gmail のヘルプに合わせて、ここでは Enter キーを使う手順を紹介します。

Gmail では「折りたたみボタン」や「メニューボタン」も登場します。
これらはボタンを押すとその場所でメニューが開いて、さらに項目を選択できるようになっています。
Gmail のメニューは上下矢印キーと Enter キーで操作できます。

リスト項目と「リンクのないテキスト」は受信トレイではなく、メールの本文が表示されるページで使います。


事前の説明はこのくらいにして、1文字ナビゲーションで「受信トレイ」のページを把握してみましょう。
Ctrl+Home でページの先頭に戻れば簡単にやり直せますね。

まず見出し(H)で大まかに移動してみましょう。
「検索」「ラベル」「ハングアウト」「メイン」が把握できましたか？

「ハングアウト」の項目は設定によって抜けていたり別の名前になっていることがありますが、今回は使いません。

次にランドマーク(D)で大まかに移動してみましょう。
「バナー」「検索」「ラベル」「ハングアウト」「メイン」のような構成です。

エディットフィールド(E)は「検索」だけ見つかると思います。
リンク(K)とボタン(B)はたくさんあるでしょう。

これらの要素の配列を独自に調査した結果を下記に紹介します。
...
- D 2 補足情報 ハングアウト
  - B チャット設定
  - B チャット相手を検索
- B メニューボタン 折りたたみ サブメニュー 選択（すべてのスレッドを選択）
...
- B 削除（スレッドを選択すると表示）
- B 移動 折りたたみ（スレッドを選択すると表示）
- B ラベル 折りたたみ（スレッドを選択すると表示）

- B その他 折りたたみ
- B xx-xx / xx ボタン 折りたたみ（表示されているメッセージの数／すべてのメッセージの数）
- B 前
- B 次
- B 設定 折りたたみ
